...
class QDDETR(nn.Module):
    ...
    def forward(self, src_txt, src_txt_mask, src_vid, src_vid_mask, vid,qid,src_aud=None, src_aud_mask=None):
        ...
        import time
        cmap = 'coolwarm'
        # Assuming hd_memory and mr_memory have the same shape[0]
        print(vid)
        # for target_vid in vid:
        # tsne_A_MFAuOwK5k_60.0_210.0(green) tsne_DTInxNfWXVc_210.0_360.0 (gold) tsne_DTInxNfWXVc_360.0_510.0 (cyan) tsne_EE0NcEec_mo_60.0_210.0 (blue)
        # tsne_h6QKDqomIPk_60.0_210.0(blueviolet) tsne_h6QKDqomIPk_360.0_510.0 (orangered) tsne_IdpHJK90VDw_210.0_360.0 (deeppink) tsne_-_s0sXOfS3w_510.0_660.0(red)
        
        # target_vids = [
        #                 'A_MFAuOwK5k_60.0_210.0','DTInxNfWXVc_210.0_360.0','DTInxNfWXVc_360.0_510.0','EE0NcEec_mo_60.0_210.0',
        #                 'h6QKDqomIPk_60.0_210.0','h6QKDqomIPk_360.0_510.0','IdpHJK90VDw_210.0_360.0','-_s0sXOfS3w_510.0_660.0',
        #                 'kMuq1MNhP00_360.0_510.0']
        # vid_colors = [
        #     'green','gold','cyan','blue','blueviolet','orangered','deeppink','red','blue'
        #     ]
        target_vids = [
                        'EE0NcEec_mo_60.0_210.0',
                        'h6QKDqomIPk_60.0_210.0',]
        vid_colors = [
            'blue','blueviolet'
            ]
        assert len(target_vids) == len(vid_colors)

        for i in range(len(target_vids)):
            target_vid = target_vids[i] #S73Z-nM0GQE_360.0_510
            color_set = vid_colors[i]  # deeppink gold orangered blueviolet blue red green cyan
            if target_vid in vid:
                index = vid.index(target_vid)
                # for i in range(src_vid.shape[0]):
                i=index

                src_vid_feats = src_vid[i].clone().detach()
                # normed_feat = calculate_l1_norm(src_event_feat)
                # mean_feat = src_event_feat.mean(-1).unsqueeze(0)
                tsne = TSNE(n_components=2, random_state=0)
                features_tsne = tsne.fit_transform(src_vid_feats.cpu())
            
                # Create a 1x2 subplot grid
                # plt.subplot(4, 1, 1)
                # plt.imshow(src_event_feat.cpu().numpy(), cmap=cmap, interpolation='nearest')
                # plt.axis('off')
                # plt.colorbar()
                # plt.title('Vid_event Feature')

                plt.subplot(2, 1, 1)
                plt.axis('off')
                plt.scatter(features_tsne[:, 0], features_tsne[:, 1], c=color_set, marker='o',s=20, alpha=0.8)
                plt.title('tsne Feature')

                plt.subplot(2, 1, 2)
                plt.axis('off')
                plt.scatter(features_tsne[:, 0], features_tsne[:, 1], c=color_set, marker='o',s=20, alpha=0.8)
                # 在每个点上标注索引
                texts = [plt.text(features_tsne[i, 0], features_tsne[i, 1], str(i), fontsize=5) for i in range(len(features_tsne))] # 创建文本标签
                from adjustText import adjust_text
                # 调整文本位置
                adjust_text(texts, force_text=(0.1, 0.1))

                plt.savefig("./visual_tsne/tsne_" + target_vid + ".png", dpi=300)
                plt.close()
        ...
        hs, reference, memory, memory_global,saved_multi_modal_rep = self.transformer(src, ~mask, self.query_embed.weight, pos, video_length=video_length)
        
        # saved_multi_modal_rep = saved_multi_modal_rep.permute(1,0,2)
        # saved_dir = '/home/yj/QD-DETR/multimodal_reps/'
