...
class CGDETR(nn.Module):
    ...
    def __init__(self, transformer, position_embed, txt_position_embed, txt_dim, vid_dim,
        ...
        self.n_mu = 35
        self.em_iter = 5
        self.mu_v1 = nn.Parameter(torch.randn(self.n_mu, vid_dim+aud_dim))
    ...
    def forward(self, src_txt, src_txt_mask, src_vid, src_vid_mask, vid, qid, src_aud=None, src_aud_mask=None, targets=None):
        ...
        ## 2.4 TODO
        # src_vid, src_txt = self.bi_cross_attn(
        #     src_vid,
        #     src_txt,
        #     mask = src_vid_mask.bool(),
        #     context_mask = src_txt_mask.bool()
        # )   

        # src_vid_event,mu = self.ema(src_vid)
        # print(src_vid_event.shape,mu.shape)

        mu = self.mu_v1[None, ...].repeat(src_vid.shape[0], 1, 1)
        em_feat,norm_latent_z = self.EM(mu, src_vid,em_iter=self.em_iter)  # 32,30,2818
        ...
        # src_vid = src_vid + self.vid_token_type_embeddings(torch.full_like(src_vid_mask.long(), 1)) #  (bsz, L_vid, d)
        # src_txt = src_txt + self.txt_token_type_embeddings(torch.zeros_like(src_txt_mask.long())) # [bsz,L_txt,d]
