...
from upm.span_utils import span_xx_to_cxw
...
class StartEndDataset(Dataset):
    ...
    def __init__(self, dset_name, data_path, v_feat_dirs, q_feat_dir,
        ...
        # self.use_glove = 'vgg' in self.v_feat_dirs[0] # for charades
        self.use_glove = 'rgb' in self.v_feat_dirs[0] # for youtube
        ...
        if self.dset_name == 'youtube_uni' and self.use_glove:
            self.vocab = vocab.pretrained_aliases['glove.6B.300d']()
            self.vocab.itos.extend(['<unk>'])
            self.vocab.stoi['<unk>'] = self.vocab.vectors.shape[0]
            self.vocab.vectors = torch.cat(
    ...
    def _get_video_feat_by_vid(self, vid):
        ...
        elif self.dset_name == 'youtube_uni' and 'rgb' in self.v_feat_dirs[0]:
            v_feat_list = []
            for _feat_dir in self.v_feat_dirs:
                _feat_path = join(_feat_dir, f"{vid}_rgb.npy")
                _feat_rgb = np.load(_feat_path)[:self.max_v_l].astype(np.float32)
        ...
        elif self.dset_name == 'youtube_uni' and 'clip' in self.v_feat_dirs[0]:
            v_feat_list = []
            for _feat_dir in self.v_feat_dirs:
