...
from upm.span_utils import generalized_temporal_iou, span_cxw_to_xx

from upm.matcher import build_matcher
from upm.transformer import build_transformer, TransformerEncoderLayer, TransformerEncoder
from upm.position_encoding import build_position_encoding
from upm.misc import accuracy
import numpy as np
import copy
...
class EMAU(nn.Module):
    ...
    Arguments:
        c_dim (int): The input and output channel number.
        k_bases (int): The number of the bases.
        stage_num (int): The iteration number for EM.
    '''
    def __init__(self, c_dim, hidden_dim, input_dropout, n_input_proj=2, k_bases=10, stage_num=3):
        super(EMAU, self).__init__()
        self.stage_num = stage_num

        mu = torch.Tensor(1, c_dim, k_bases)
        mu.normal_(0, math.sqrt(2. / k_bases))    # Init with Kaiming Norm.
        ...
        self.input_proj = nn.Sequential(*[
            LinearLayer(c_dim, hidden_dim, layer_norm=True, dropout=input_dropout, relu=relu_args[0]),
    ...
    def forward(self, x):
        ...
        # The EM Attention
        b, l, c = x.size()                  # b,l,c/dim
        x = x.transpose(1,2)                # b,c/dim,l
...
def random_walk(x, y, w):
    ...
    # 检查结果是否接近 A 的逆
    # A_inverse = torch.inverse(eye_x - (w ** 2) * affinity_mat)
    # max_error = torch.max(torch.abs(A_inverse - mat_inv_x))
...
class CGDETR(nn.Module):
    ...
    def __init__(self, transformer, position_embed, txt_position_embed, txt_dim, vid_dim,
        ...
        # self.vid_token_type_embeddings.apply(init_weights)
        # self.txt_token_type_embeddings.apply(init_weights)
        self.use_txt_pos = use_txt_pos
        self.n_input_proj = n_input_proj
        self.query_embed = nn.Embedding(num_queries, 2)

        # self.ema = EMAU(hidden_dim,hidden_dim,input_dropout,n_input_proj=1,k_bases=30,stage_num=5)
        
        # self.stage_num = 5
        # self.k_bases = 30
        # mu = torch.Tensor(1, hidden_dim, self.k_bases)
        # mu.normal_(0, math.sqrt(2. / self.k_bases))    # Init with Kaiming Norm.
        # mu = self._l2norm(mu, dim=1)
        # self.register_buffer('mu', mu)
        ...
        self.n_mu = 35
        self.em_iter = 5
        self.mu_v1 = nn.Parameter(torch.randn(self.n_mu, vid_dim))
        # torch.nn.init.xavier_uniform_(self.mu_v1)

        self.n_mu_txt = 5
        self.em_iter_txt = 5
        self.mu_txt = nn.Parameter(torch.randn(self.n_mu_txt, txt_dim))

        if aud_dim != 0 and not self.args.audio_cat:
            self.n_mu_audio = 35
            self.em_iter_audio = 5
            self.mu_audio = nn.Parameter(torch.randn(self.n_mu_audio, aud_dim))

        torch.nn.init.xavier_uniform_(self.mu_v1)
        torch.nn.init.xavier_uniform_(self.mu_txt)
        if aud_dim != 0:
            torch.nn.init.xavier_uniform_(self.mu_audio)
        # self.em_vid_proj = nn.Sequential(*[
    ...
    def EM(self, mu, x,em_iter):
        ...
        for _ in range(em_iter):
            ...
            # rbf kernel
            latent_z = F.softmax(torch.exp(-1.0 * torch.cdist(norm_mu, norm_x, p=2)**2),1)
            norm_latent_z = latent_z / (latent_z.sum(dim=-1, keepdim=True)+1e-9)
            mu = torch.einsum('nkt,ntd->nkd', [norm_latent_z, x])
        return mu,norm_latent_z
    ...
    def forward(self, src_txt, src_txt_mask, src_vid, src_vid_mask, vid, qid, src_aud=None, src_aud_mask=None, targets=None):
        ...
        mu = self.mu_v1[None, ...].repeat(src_vid.shape[0], 1, 1)
        em_feat,norm_latent_z = self.EM(mu, src_vid,em_iter=self.em_iter)  # 32,75,2818
        src_vid_event = my_random_walk(src_vid, em_feat, 0.5,lambda_z=5.0) # self.w

        mu_txt = self.mu_txt[None, ...].repeat(src_txt.shape[0], 1, 1)
        em_feat_txt,norm_latent_z_txt = self.EM(mu_txt, src_txt,em_iter=self.em_iter_txt)  # 32,30,2818
        src_txt_phrase = my_random_walk(src_txt, em_feat_txt, 0.5,lambda_z=5.0) # self.w


        src_vid = self.input_vid_proj(src_vid) + self.input_vid_proj(src_vid_event)
        src_txt = self.input_txt_proj(src_txt) + self.input_txt_proj(src_txt_phrase)

        if src_aud is not None:
            mu_audio = self.mu_audio[None, ...].repeat(src_aud.shape[0], 1, 1)
            em_feat_audio,norm_latent_z_audio = self.EM(mu_audio, src_aud,em_iter=self.em_iter_audio)  # 32,75,2050
            src_audio_event = my_random_walk(src_aud, em_feat_audio, 0.5,lambda_z=5.0) # self.w
            
            src_aud = self.input_audio_proj(src_aud) + self.input_audio_proj(src_audio_event)
        ...
        if self.args.global_cal_type == 'random_walk':
            vid_txt_sim = torch.matmul(src_vid,src_txt.permute(0,2,1))
            ...
            for i in range(len(src_vid)):
                vidsrc_[i] = src_vid[i][topkidx[i][-1]]
        ...
        if targets is not None: ## train
            ...
            hs, reference, memory, memory_global, attn_weights, memory_moment, nmmemory_moment, mmemory_frames, nmmemory_frames,saved_multi_modal_rep = self.transformer(src, ~mask, self.query_embed.weight, pos, video_length=video_length, moment_idx=targets["relevant_clips"], msrc=msrc, mpos=mpos, mmask=~mmask, nmsrc=nmsrc, nmpos=nmpos, nmmask=~nmmask,
            ...
            hs, reference, memory, memory_global, attn_weights, memory_moment, nmmemory_moment, mmemory_frames, nmmemory_frames,saved_multi_modal_rep = self.transformer(src, ~mask, self.query_embed.weight, pos, video_length=video_length,
                                                                                                         ctxtoken=vidsrc_, gtoken=self.global_rep_token, gpos=self.global_rep_pos, vlen=src_vid_mask.sum(1).long())
        
        from utils.basic_utils import mkdirp
        mkdirp('./multimodal_reps/reps')
        mkdirp('./multimodal_reps/rep_masks')
        saved_dir = './multimodal_reps/'
        for i in range(len(vid)):
            vid_name = vid[i]
            vid_mask = src_vid_mask[i].cpu().numpy()
            vid_rep = saved_multi_modal_rep[i].cpu().numpy()
            print(vid_mask.shape,vid_rep.shape)
            np.savez(saved_dir+'rep_masks/'+vid_name+'_'+str(qid[i])+'.npz', data=vid_mask)
            np.savez(saved_dir+'reps/'+vid_name+'_'+str(qid[i])+'.npz', data=vid_rep)
        ...
        if vid is not None: ## for demo (run_on_video/run.py)
            ...
            if real_neg_mask.sum() != 0:
                ...
                _, _, memory_neg, memory_global_neg, attn_weights_neg, _, _, _, _, _ = self.transformer(src_neg, ~mask_neg, self.query_embed.weight, pos_neg, video_length=video_length,
